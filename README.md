# M3U8 代理服务器 (Pro)

一个工业级的 M3U8 流媒体代理服务器，集成 AI 赋能的广告检测、多源数据融合、高性能缓存以及现代化的管理面板。

## 📋 项目概述

M3U8 Proxy Server 是一个高性能的 Node.js 流媒体中间件，专为广告过滤和流量优化而设计。它不仅支持传统的正则匹配，还引入了基于 **TensorFlow.js** 的神经网络检测模型和 TS 元数据深度分析，能有效识别并剔除流媒体中的动态广告内容。

## 🚀 核心功能特性

### 🤖 AI 赋能检测
- **神经网络分类器**：基于 TensorFlow.js 的自定义模型，通过学习片段特征识别广告。
- **在线训练系统**：内置模型训练页面，支持通过实时数据不断优化检测准确率。
- **多特征输入**：分析片段时长、码率变化、分辨率异常等 20+ 个维度特征。

### 🧠 多源数据融合引擎
- **加权评分系统**：综合正则匹配、结构分析、TS 头检测和 AI 预测的多个维度进行打分。
- **硬规则保护**：对于高置信度的检测结果实现快速决策，降低处理延迟。
- **上下文感知**：结合播放列表的前后文关联性，精准识别插入式广告。

### ⚡ 工业级缓存与性能
- **智能双层缓存**：内存 LRU 缓存 + 自动持久化存储，支持秒级恢复。
- **动态 TTL 策略**：根据访问频率和文件大小自动调整缓存生命周期。
- **异步处理流水线**：非阻塞式 TS 探测，确保播放流畅度不受影响。

### 🛡️ 全方位的安全与管理
- **JWT 身份认证**：安全的管理面板访问控制。
- **实时监控面板**：直观的仪表板展示实时流量、过滤率和服务器负载。
- **配置热更新**：支持通过 Web UI 动态调整过滤强度和系统参数。

## 🛠 技术栈

### 后端核心
- **Node.js & Express** - 核心运行时与 Web 框架
- **TensorFlow.js** - 机器学习引擎
- **mpegts.js / ts-demuxer** - TS 流深度解析
- **jsonwebtoken** - 安全身份验证
- **Axios** - 高性能 HTTP 请求

### 前端管理面板
- **React 18 & TypeScript** - 现代前端开发栈
- **Vite** - 极速构建工具
- **Tailwind CSS** - 现代化响应式布局
- **TanStack Query** - 强大的异步状态管理
- **HLS.js** - 工业级流媒体播放测试

## 📦 环境要求

- **Node.js**: >= 16.0.0 (推荐 v18+)
- **npm**: >= 7.0.0

## 🚀 快速开始

### 1. 克隆项目
```bash
git clone https://github.com/levarsi/m3u8-proxy.git
cd m3u8-proxy
```

### 2. 安装依赖
```bash
# 安装后端依赖
npm install

# 安装前端依赖
npm run ui:install
```

### 3. 启动服务

#### 开发模式
```bash
# 启动后端服务（开发模式，支持热重载）
npm run dev

# 启动前端开发服务器（新终端窗口）
npm run ui:dev
```

#### 生产模式
```bash
# 构建前端资源
npm run ui:build

# 启动生产服务器
npm start
```

## ⚙️ 配置说明

### 环境变量配置
创建 `.env` 文件或设置环境变量：

```bash
# 服务器配置
PORT=3000                          # 服务端口
HOST=127.0.0.1                    # 监听地址
NODE_ENV=production                # 运行环境

# 广告过滤配置
AD_FILTER_ENABLED=true             # 启用广告过滤
AD_FILTER_LOG_LEVEL=info          # 过滤日志级别

# 缓存配置
CACHE_ENABLED=true                 # 启用缓存
CACHE_TTL=300000                   # 缓存有效期（毫秒）
CACHE_MAX_SIZE=100                 # 最大缓存条目数

# 安全配置
RATE_LIMIT_ENABLED=true            # 启用速率限制
RATE_LIMIT_WINDOW=60000            # 速率限制窗口（毫秒）
RATE_LIMIT_MAX=100                 # 窗口内最大请求数

# CORS 配置
CORS_ENABLED=true                  # 启用 CORS
CORS_ORIGIN=*                      # 允许的源
```

### 3. 访问与登录
- **管理后台**: `http://localhost:3000`
- **默认账号**: `admin`
- **默认密码**: `admin`

## ⚙️ 核心配置 (`config.js`)

```javascript
module.exports = {
  // 认证配置
  auth: {
    enabled: true,
    username: 'admin',
    password: 'admin', // 生产环境请务必修改
    jwtSecret: 'your-secret-key'
  },
  
  // 广告过滤深度配置
  adFilter: {
    enabled: true,
    enableTSDetection: true, // 启用 TS 深度检测
    tsDetection: {
      confidenceThreshold: 0.9, // 判定阈值
      concurrencyLimit: 5      // 并发检测限制
    }
  },
  
  // 缓存策略
  cache: {
    enabled: true,
    ttl: 600000,
    persistence: {
      enabled: true,
      filePath: './data/cache-data.json'
    }
  }
};
```

## 🌐 API 接口

### 核心代理接口
```http
GET /proxy?url={m3u8_url}
```
代理和过滤 M3U8 播放列表

**参数**:
- `url` (必需) - 目标 M3U8 文件的 URL

**响应头**:
- `Content-Type: application/vnd.apple.mpegurl`
- `X-Processed-By: M3U8-Proxy`
- `X-Processing-Time` - 处理耗时
- `X-Segment-Count` - 片段数量

### 系统管理接口
```http
GET  /health              # 系统健康检查
GET  /cache/stats         # 缓存统计信息
GET  /cache/clear         # 清除缓存
GET  /logs                # 获取系统日志
POST /api/settings        # 更新系统配置
```

### 广告过滤接口
```http
GET  /ad-filter/rules           # 获取过滤规则
POST /ad-filter/rules          # 添加过滤规则
GET  /ts-detector/stats        # TS 检测统计
POST /ts-detector/config       # 更新检测配置
```

### 测试接口
```http
GET /mock-stream.m3u8    # 获取包含广告的测试流
```

## 📖 使用示例

### 1. 代理 M3U8 流
```bash
# 基础代理
curl "http://localhost:3000/proxy?url=https://example.com/stream.m3u8"

# 使用测试流验证广告过滤
curl "http://localhost:3000/proxy?url=http://localhost:3000/mock-stream.m3u8"
```

### 2. 系统状态查询
```bash
# 健康检查
curl http://localhost:3000/health

# 缓存状态
curl http://localhost:3000/cache/stats
```

### 3. 前端界面使用
1. 访问 http://localhost:3000
2. 在"测试/播放"页面输入 M3U8 URL
3. 查看过滤结果和统计信息
4. 使用内置播放器测试播放效果

### 4. 自定义广告过滤规则
```javascript
// 在 config.js 中添加自定义规则
adFilter: {
  customPatterns: [
    /custom_ad_pattern/i,
    /specific_brand/i
  ]
}
```


## 🏗️ 项目结构

```text
m3u8-proxy/
├── server.js                # 主入口，处理路由与认证
├── m3u8-processor.js        # 核心逻辑：解析与过滤 M3U8
├── ts-metadata-detector.js  # TS 片段特征提取器
├── neural-network-model.js  # AI 模型定义与预测
├── multi-source-fusion.js   # 多维度决策融合引擎
├── stats-manager.js         # 全局统计与持久化
├── cache-manager.js         # 智能缓存管理
├── config.js                # 集中化配置文件
├── data/                    # 持久化数据存储 (JSON, 模型)
├── frontend/                # React 管理后台源码
└── test/                    # 完整的测试套件
```

## 🧪 自动化测试

项目包含完善的测试流程，确保检测引擎的准确性：
```bash
node test/test-ts-detection.js   # 测试 TS 元数据检测逻辑
node test/test-learning.js       # 测试 AI 学习与模型更新
node test/test-integration.js    # 系统集成压力测试
```

## 📊 路线图 (Roadmap)
- [x] 基于神经网络的广告识别 (Alpha)
- [x] 多源检测数据融合
- [x] 现代化管理后台
- [ ] 分布式缓存支持 (Redis)
- [ ] 视频指纹识别技术
- [ ] 容器化部署支持 (Docker)

## 📄 许可证
本项目基于 [MIT](LICENSE) 许可证开源。

---
**提示**: 本项目仅供学习与研究使用，请在遵守当地法律法规的前提下使用代理技术。
